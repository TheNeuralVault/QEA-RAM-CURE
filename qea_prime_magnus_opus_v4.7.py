#!/usr/bin/env python3
"""
QEA PRIME v4.7 — RAM-AWARE PHONE-OPTIMIZED
"""
...
def self_diagnose_and_heal():
    ...
    if ram < 600:
        print("  [SELFHEALER] Low RAM — killing background processes")
...
def run_cycle(cycle):
    ...
    print(f"\n{'═'*100}\nQEA PRIME v4.7 | CYCLE {cycle} | RAM {ram} MB | LIVE HARDWARE + GLOBAL | {datetime.now().strftime('%H:%M:%S')}")

    # Auto-scale qubit count based on RAM
    if ram < 1200:
        max_qubits = 9
    elif ram < 2000:
        max_qubits = 11
    ...
    if cycle % 5 == 0:
        ...
        HUMAN_REVIEW.write_text(f"=== v4.7 PROPOSAL — Cycle {cycle} ===\n{improvement}")
        human_gate(cycle)

    with open(LEDGER, 'a') as f:
        f.write(f"\n### CYCLE {cycle} | {datetime.now().strftime('%Y-%m-%d %H:%M')}\n{status}\n---\n")

    print(f"  [CYCLE {cycle}] COMPLETE — phone-safe")

def main():
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║           Q E A   P R I M E   v 4 . 7                       ║")
    print("║     RAM-AWARE PHONE-OPTIMIZED + SELF-HEALING                ║")
    print("╚══════════════════════════════════════════════════════════════╝")
    ...
    while True:
        cycle += 1
        run_cycle(cycle)
        print("  [SLEEP] 10 minutes — phone cooling & recharging")
        time.sleep(600)
